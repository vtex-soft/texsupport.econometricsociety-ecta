...
Try to avoid excessive use of italics and bold face; underlining is generally banned (except for exceptional cases). Please do not use any \LaTeXe\ or \TeX\ commands that affect the layout or formatting of your document (i.e., commands like \verb|\textheight|, \verb|\textwidth|, etc.). Note that the Introduction should be Section~\ref{s1} it should not imediately follow the abstract without a heading. If your paper has a an approved supplement, reference the supplement at the end of the Introduction.  Cite the supplement;  \textit{Example}: The Supplemental Appendix (\citet{b2}) contains results for empirical tests. 
When referencing material from the Supplement, use plain text.
...
Multiple bibliography items cite: \citet{b2,b3,b5,b6}.
Author only cite: \citeauthor{b3}.
Year only cite: (\citeyear{b3}). Citing bibliography with object \citet[Theorem 1]{b1}. Citing within brackets is done with the same commands (e.g., \citet{b2,b3,b5}). For papers with 5 or more authors, use the initials of the first four authors and the + symbol. For example, \citet{b4}.
...
and also in Table~\ref{parset}. Vertical lines in tables are not permitted by journal style. 

Sample of cross-reference to figure: Figure~\ref{penG} shows that it is not easy to get something on paper. Note that figures will be in grayscale in the printed version.  Color should not be mentioned in caption or text. 
\begin{table*}
...
\hline
\end{tabular}
\legend{This is how a table note should be presented, not as part of the table title. Please do not use asterisks or bold face to denote statistical significance. We encourage authors to report standard errors and coverage sets or confidence intervals.}
...
\caption{The typesetter will convert the figures to grayscale for the print version, so please use dashed/dotted lines or symbols where possible and avoid referring to colors in the text and the figure caption.  Numbers less than zero should have a leading zero in front of the decimal point. Note(s) should be added to the figure caption. }
\label{penG}
\end{figure}

\begin{figure}
\includegraphics{sample2}
\caption{Grouped bar charts can use patterns.}
\label{penG}
\end{figure}
...
These notes apply to an Appendix within the paper, not a Supplemental Appendix. Appendices should be provided in \verb|{appendix}| environment. If there is only one appendix,
...
%\bibliographystyle{econsoc} % Style BST file
...
\bibitem[\protect\citeauthoryear{Cahuc, Postel-Vinay and Robin}{2006}]{b2}
Cahuc, Pierre, Fabien Postel-Vinay, and Jean-Marc Robin (2006), ``Supplement to `Wage Bargaining with On-the-Job Search: Theory and Evidence'.''
...
\bibitem[\protect\citeauthoryear{\xcitelabel{HNPS+}}{2023}]{b4}
Horta\c{c}su, Ali, Olivia~R. Natan, Hayden Parsley, Timothy Schwieg, and Kevin~R. Williams (2023),
``Demand Estimation with Infrequent Purchases and Small Market Sizes.''
\textit{Quantitative Economics}, 14 (4), 1251--1294.
\endbibitem


\bibitem[\protect\citeauthoryear{Peck}{1994}]{b5}
Peck, James (1994),
